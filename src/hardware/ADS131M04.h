...
public:
    ...
    uint8_t ADS131M04_RST_PIN;
    uint8_t ADS131M04_CLK_PIN;
    uint8_t ADS131M04_SCK_PIN;
    uint8_t ADS131M04_MISO_PIN;
    uint8_t ADS131M04_MOSI_PIN;

    void begin(uint8_t clk_pin, uint8_t sck_pin, uint8_t miso_pin, uint8_t mosi_pin, uint8_t cs_pin,
               uint8_t drdy_pin, uint8_t rst_pin);
    int8_t isDataReadySoft(byte channel);
    bool isDataReady(void);
    bool isResetStatus(void);
    bool isLockSPI(void);
    bool setSPILock(bool lock);
