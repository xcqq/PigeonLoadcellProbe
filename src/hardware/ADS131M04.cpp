...
{
    // Escribe un valor en el registro, aplicando la mascara para tocar unicamente los bits
    // necesarios. No realiza el corrimiento de bits (shift), hay que pasarle ya el valor corrido a
    // la posicion correcta

    // Leo el contenido actual del registro
    uint16_t register_contents = readRegister(address);

    // Cambio bit aa bit la mascara (queda 1 en los bits que no hay que tocar y 0 en los bits a
    // modificar) Se realiza un AND co el contenido actual del registro.  Quedan "0" en la parte a
    // modificar
    register_contents = register_contents & ~mask;

    // se realiza un OR con el valor a cargar en el registro.  Ojo, valor debe estar en el posicion
    // (shitf) correcta
...
void ADS131M04::begin(uint8_t clk_pin, uint8_t sck_pin, uint8_t miso_pin, uint8_t mosi_pin,
                      uint8_t cs_pin, uint8_t drdy_pin, uint8_t rst_pin)
{
    // Set pins up
    ADS131M04_CS_PIN = cs_pin;
    ADS131M04_DRDY_PIN = drdy_pin;
    ADS131M04_RST_PIN = rst_pin;
    ADS131M04_CLK_PIN = clk_pin;
    ADS131M04_SCK_PIN = sck_pin;
    ADS131M04_MISO_PIN = miso_pin;
    ADS131M04_MOSI_PIN = mosi_pin;

    SPI1.setTX(ADS131M04_MOSI_PIN);
    SPI1.setRX(ADS131M04_MISO_PIN);
    SPI1.setSCK(ADS131M04_SCK_PIN);
    SPI1.begin();
    SPI1.beginTransaction(settings);
    // Configure chip select as an output
    pinMode(ADS131M04_CS_PIN, OUTPUT);
    digitalWrite(ADS131M04_CS_PIN, HIGH); // Set CS high to deselect the chip
    // Configure DRDY as as input
    pinMode(ADS131M04_DRDY_PIN, INPUT);
    pinMode(ADS131M04_CLK_PIN, OUTPUT);
    // Configure reset pin as an output
    pinMode(ADS131M04_RST_PIN, OUTPUT);
    digitalWrite(ADS131M04_RST_PIN, HIGH);
    delay(100);
    digitalWrite(ADS131M04_RST_PIN, LOW);
    delay(100);
    digitalWrite(ADS131M04_RST_PIN, HIGH);
    delay(100);
...
bool ADS131M04::isResetStatus(void) { return (readRegister(REG_STATUS) & REGMASK_STATUS_RESET); }

bool ADS131M04::isLockSPI(void) { return (readRegister(REG_STATUS) & REGMASK_STATUS_LOCK); }
...
bool ADS131M04::setSPILock(bool lock)
{
    digitalWrite(ADS131M04_CS_PIN, LOW);
    delayMicroseconds(1);
    if (lock) {
        SPI1.transfer(CMD_LOCK >> 8);
        SPI1.transfer(CMD_LOCK & 0xFF);
        SPI1.transfer(0x00);
    } else {
        SPI1.transfer(CMD_LOCK >> 8);
        SPI1.transfer(CMD_LOCK & 0xFF);
